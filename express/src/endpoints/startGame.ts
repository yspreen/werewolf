...
export async function startGame(req: Request, res: Response) {
  ...
  if (room.memberIds.length !== Object.values(roleCount).reduce((a, b) => a + b, 0))
  ...
  const thiefCount = roleCount[Role[Role.THIEF]] ?? 0
  for (let i = 0; i < thiefCount; i++) {
    choices.push(Role[Role.VILLAGER])
    choices.push(Role[Role.VILLAGER])
  }
  shuffle(choices)
